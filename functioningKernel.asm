...
*******NOTE THAT THE "FUNCTION HEADERS" (i.e. the description above each function) ARE NOT NECESSARILY CORRECT (although all the functions should work as necessary)
...
;;; ; ; ; ;   [%FP - 12]: G4
;;; ; ; ; ; Parameters:
;;; ; ; ; ;   [%FP + 0]: boolean exit: if this value is 1, the call to scheduler is coming from EXIT, otherwise it's coming from clock interrupt
;;; ; ; ; ; Caller preserved registers:
;;; ; ; ; ;   [%FP + 12]: FP
;;; ; ; ; ; Return address:
;;; ; ; ; ;   [%FP + 8]
...
clock_int_handler:
	...
	COPY		*+fpTemp			%FP
	COPY		*+spTemp			%SP ;save stack and frame pointer cos we need to input them to process table
...
_clk_alm2:				0x4		;second word of two-word value containing how many instructions to process before throwing an interrupt
...
_string_banner_msg:		"kernel\n"
_string_copyright_msg:		"(c) ConnerReilly/TomalHossain/Mohammed Ibrahim \n"
_string_done_msg:		"done.\n"
_string_abort_msg:		"failed!  Halting now.\n"
_string_initializing_init:	"We are about to JUMPMD into init.vmx"
_string_blank_line:		"                                                                                "
_string_return_to_init:		"We are returning to init after a CREATE system call."
;;; ; ================================================================================================================================
